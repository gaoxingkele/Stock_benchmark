...
- **Implication**: Are Transformers Effective for Time Series Forecasting? should be translated into a falsifiable proxy module before any production claim.

### O2: The paper belongs to the `decomposition_linear` family
- **Statement**: Local survey metadata classifies this candidate as `decomposition_linear`.
- **Evidence**: `papers/metadata/lingxi_cross_domain_ts_sota_candidates.csv`
- **Implication**: The first local validation should test the corresponding Lingxi transfer idea: `LinearGuard baseline every neural upgrade must beat`.
