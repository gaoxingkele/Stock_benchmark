...
- **Implication**: iTransformer: Inverted Transformers Are Effective for Time Series Forecasting should be translated into a falsifiable proxy module before any production claim.

### O2: The paper belongs to the `variate_attention` family
- **Statement**: Local survey metadata classifies this candidate as `variate_attention`.
- **Evidence**: `papers/metadata/lingxi_cross_domain_ts_sota_candidates.csv`
- **Implication**: The first local validation should test the corresponding Lingxi transfer idea: `Cross-sectional variate-attention ranking proxy`.
