...
- **Implication**: Non-stationary Transformers: Exploring the Stationarity in Time Series Forecasting should be translated into a falsifiable proxy module before any production claim.

### O2: The paper belongs to the `nonstationarity_correction` family
- **Statement**: Local survey metadata classifies this candidate as `nonstationarity_correction`.
- **Evidence**: `papers/metadata/lingxi_cross_domain_ts_sota_candidates.csv`
- **Implication**: The first local validation should test the corresponding Lingxi transfer idea: `Point-in-time normalization and de-stationary score restoration`.
