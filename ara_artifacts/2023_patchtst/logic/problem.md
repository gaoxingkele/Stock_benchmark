...
- **Implication**: PatchTST: A Time Series is Worth 64 Words should be translated into a falsifiable proxy module before any production claim.

### O2: The paper belongs to the `patch_transformer` family
- **Statement**: Local survey metadata classifies this candidate as `patch_transformer`.
- **Evidence**: `papers/metadata/lingxi_cross_domain_ts_sota_candidates.csv`
- **Implication**: The first local validation should test the corresponding Lingxi transfer idea: `Patch-token OHLCV feature proxy for Lingxi10/Lingxi5`.
